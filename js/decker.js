...
TOOLS=image_tiles(12,16,image_read(
	...
	'K33O5WqxpVpwjyzqsUooJroNn7bjNRyPB7To6lz1A+L7wXRZOhR4hj6HeYmDQFNqcWhyZJeJUjR7WIY3ZZtLlkubImZlll2O'+
	'LGWUQUOdk6lNr5daciW7s6C+uquwvolqRKq8QiGArcITfY+3IKeEMySgndI9Y0QfdTevY8aOPszBsNpTdcRtsyTG13sn5+zn'+
	'6S7t7+Li9Pb00fTFXfN98pDS5gtCxZXsU5aDAhJIVsDhIsKDBil4cE87UDYW9eRngYLZrzqGUjOZHqSHYUSRGixJUsK/Uhpc'+
	'jHpx8IF6qZ+Qcnn0upXLL8ObGOOHY2LI4zCpKjyY8ok4pCioWfE0csCwA='
))
ARROWS=image_tiles(8,12,image_read(
	'%%IMG3AAwAYAKEj2kReg1gknEqSVFjTPOnXaEFTlznpKp5iuW4uPHyPV/24hmFV76DWgmFwUuNdaSdgstlpOlUJqVFqi46zN'+
	'payOqT6wFDnWMmtNsDG7Wqnq39I2HTM5cbAxxhY24acdXnB7fDxZNXJ5cCQUeFGNLnBQPXBhn5NTdmVVi21RS29wUYGCejGD'+
	'iV2YmZWraYeqkWRgmoUAA='
))
...
GESTURES={
	left: image_read('%%IMG3ABAAEAZAgHBILBqLoCToOFQGAsuj8wk1TqnVZhKLVV65XRB4DP2So2aqkpiOWrdqpnDqlnvl+DwxCA=='),
	right:image_read('%%IMG3ABAAEAZAgHBILA5BSJDRCAoEksuj0wldNqdUJDGJxVav3W4SHBaTy8/xeVoVrttSttaaVUYB47t+zw8C'),
	up:   image_read('%%IMG3ABAAEAZAgHAIABmJSCIoAEomjcymc7gMMKdCqPU4rVqvTu2Xi/R+wUrzWEo9u9lF9Zksf7fd9qxRvCcrx1h6e0hB'),
	down: image_read('%%IMG3ABAAEAZAgFAIKhaHSCQowAQlicZlMzqUMq9YJxHLbVat3WO1e9V+yWIl+gkoctNJcMD8dE/ZYy8eOqfjl35/cENB'),
}
RADIOS=[
...
HANDLES=[
	'%%IMG3AAcACgZAQGA4FIKOxyEyGViClEsoUpo0VgHGAHarxQYB',
	'%%IMG3AAoABwZAQGBIBBgDoKQycFQ6mUjnMiqFSkHMI3FoBAQB',
].map(image_read)
...
modal_enter=type=>{
	...
	if(type=='action'){
		...
		ms.carda=image_read('%%IMG3ABEADQKEj6nLGu3Ag2ZKeiu1uoUPconYReaJkNi6FA==')
		ms.cardb=image_read('%%IMG3ABEADQKMj6nLBg+bS0BWdBncWfXzgUs4edqocY/EtkoB')
...
modals=_=>{
	...
	if(ms.type=='about'){
		...
		if(ui_button(cb,copy_message,can_copy&&ms.grid.row>-1)){
			if(patterns_is(sel)){const dst=ifield(deck,'patterns');for(let z=2;z<32+PAL_COLORS;z++)iindex(dst,z,iindex(sel,z))}
		...
		if(sel&&patterns_is(sel)){
			...
			const draw_pattern=(pix,x,y)=>pix<2?(pix?1:0): pix>31?(pix==32?0:1): pal_pat(pal,pix,x,y)&1
			const draw_color  =(pix,x,y)=>pix==ANTS?anim_ants(x,y): pix>=32+PAL_COLORS?0: pix>31?pix-32: draw_pattern(pix,x,y)?15:0
		...
		const grid=rect(8,4+0|(PAL_COLORS/8)), ss=25, gs=ss+4, m=5, lh=font_h(FONT_BODY)
...
toolbars=_=>{
	...
	const toolbar=(enable,element,render,behavior)=>{
		...
		const draw_pattern=(pix,x,y)=>pix<2?(pix?1:0): pix>31?(pix==32?0:1): pal_pat(pal,pix,x,y)&1
		const draw_color  =(pix,x,y)=>pix>=32+PAL_COLORS?0: pix>31?pix-32: draw_pattern(pix,x,y)?15:0
