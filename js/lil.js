...
monad={
	...
	count:  x=>lmn(count(x)),
	first:  x=>lit(x)?monad.first(rows(x)): lion(x)?lms(x.n): count(x)?ll(x)[0]: NONE,
...
image_merge_op=(target,src,op)=>{
	const ts=target.size,bs=src.size,t=target.pix,b=src.pix
...
image_make=size=>{
	const f=(self,i,x)=>{
		...
		if(ikey(i,'translate'))return lmnat(([x,y])=>{
			const o=getpair(x), w=y?lb(y):0;if(o.x==0&&o.y==0)return self;const s=self.size,t=image_make(s)
