...
int main(int argc,char**argv){
	...
	unsigned char*b=malloc(st.st_size);if(fread(b,1,st.st_size,f)!=(unsigned)st.st_size){fclose(f);fprintf(stderr,"read failure.\n");exit(1);}fclose(f);

	char vn[4096]={0};unsigned int l=strlen(fn);for(unsigned int z=0;z<sizeof(vn)-1&&z<l;z++){char c=fn[z];vn[z]=c=='.'?'_': c=='/'?'_': c=='\\'?'_': c;}
	printf("unsigned int %s_len=%lld;\n",vn,st.st_size);
	printf("unsigned char %s[]={",vn);
	for(unsigned int z=0;z<st.st_size;z++){if(z!=0)printf(",");printf("0x%02x",b[z]);}
	printf("};\n");
	return 0;
}
