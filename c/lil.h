...
monad(l_raze){
	...
	lv*f=l_first(x);
	if(lis(f)||lin(f)){int a=1;for(int z=1;z<x->c;z++)if(!lis(x->lv[z])&&!lin(x->lv[z])){a=0;break;};if(a)return x;}
...
lv* pjson(char*t,int*i,int*f,int*n){
	...
	char tb[NUM];int nl=MIN(*i-ns,NUM-1);memcpy(tb,t+ns,nl);tb[nl]='\0';return lmn(atof(tb));
}
...
void format_type(str*r,lv*a,char t,int n,int d,int lf,int pz,int*f,char*c){
	...
	if     (t=='%')snprintf(o,NUM,"%%");
		...
		time_t v=ln(a);struct tm*tv=gmtime(&v);
...
lv* time_zone_offset(void){
	time_t t=time(NULL);struct tm*utc_tm=gmtime(&t);utc_tm->tm_isdst=-1;time_t utc_epoch=mktime(utc_tm);
