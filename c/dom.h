...
lv* n_image_merge(lv*self,lv*z){
	...
	for(int p=0;p<z->c;p++)if(image_is(z->lv[p])&&!is_empty(z->lv[p]))v[p]=1,ss[p]=image_size(z->lv[p]),b[p]=z->lv[p]->b->sv;
...
lv* n_canvas_merge(lv*self,lv*z){
	...
	for(int p=0;p<z->c;p++)if(image_is(z->lv[p])&&!is_empty(z->lv[p]))v[p]=1,s[p]=image_size(z->lv[p]),b[p]=z->lv[p]->b->sv;
