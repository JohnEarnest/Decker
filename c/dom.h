...
lv* event_invokev(lv*target,lv*name,lv*arg,lv*hunk,int nodiscard){
	...
	for(int z=scopes->c-1;z>=0;z--){
		...
		if(deck_is(t)){
			lv*mv=ifield(t,"modules"),*modules=lmd();EACH(z,mv)dset(modules,mv->kv[z],ifield(mv->lv[z],"value"));
			blk_locs(b,modules);
...
char base64_enc[66]="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=";
signed char base64_dec[256]={
...
lv* base64_read(lv*x,int base){
	lv*r=lms(3*(x->c/4));r->c=0;for(int z=base;z<x->c;z+=4){
		int c[4]={0},o=0;for(;o<4&&z+o<x->c;o++){c[o]=base64_dec[0xFF&x->sv[z+o]];if(c[o]==-1){z=x->c;break;}}
...
void draw_line_function(rect r,lv*func,int pattern){
	lv*a=lml3(lmpair((pair){r.w-r.x,r.h-r.y}),ONE,LNIL),*p=lmblk(),*e=lmenv(NULL);blk_lit(p,func),blk_lit(p,a),blk_op(p,CALL),pushstate(e);
	int dx=abs(r.w-r.x), dy=-abs(r.h-r.y), err=dx+dy, sx=r.x<r.w ?1:-1, sy=r.y<r.h?1:-1;while(!do_panic){
		if(func->c>2)a->lv[2]=lmpair((pair){r.x,r.y});
...
lv* interface_danger(lv*self,lv*i,lv*x){
	...
	ikey("open"    )return lmnat(n_danger_open,self);
#ifndef _WIN32
